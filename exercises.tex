...
\end{exer}
\begin{proof}
The class \(C=\{\,x\mid x=\emptyset\text{ or }\exists y(x=\suc{y})\,\}\) is obviously inductive, so \(X\sect C\)~is inductive. Taking \(X=\N\), the rest follows, with \(m\in\N\) by transitivity of~\(\N\).
\end{proof}
