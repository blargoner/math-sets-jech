...
\begin{exer}[4.9]
If \(P_2\not\subset P_1\) are perfect sets, then \(\card{P_2-P_1}=\cc\).
\end{exer}
\begin{proof}
If \(x\in P_2-P_1\), then by closure of~\(P_1\) there is~\((a,b)\) with \(x\in(a,b)\) and \(P_1\sect(a,b)=\emptyset\). Now \(P_2\sect(a,b)\ne\emptyset\) and \(P_2\sect(a,b)\subset P_2-P_1\), so \(\cc=\card{P_2\sect(a,b)}\le\card{P_2-P_1}\) (Exercise~4.8).
\end{proof}
