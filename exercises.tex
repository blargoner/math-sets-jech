...
\item[(iv)] If \(f:n\to B\) is surjective, define \(g:B\to n\) by letting \(g(y)\)~be the least \(m\in n\) such that \(f(m)=y\). Then \(g\)~is injective, so \(B\)~has the same cardinality as a subset of~\(n\), and hence \(\card{B}\le n\) by~(i). The result follows.\qedhere
\end{enumerate}
\end{proof}
