...
\section*{Chapter~2}
\begin{rmk}
In~(2.1), \(<\)~is actually a \emph{well-ordering} on~\(\Ord\). Indeed, if \(C\subclass\Ord\) is a nonempty class of ordinals and \(\alpha\in C\) is not \(\in\)-minimal in~\(C\), then \(\alpha\sect C\)~is a nonempty subset of~\(\alpha\) which has an \(\in\)-minimal element~\(\beta\). By transitivity of~\(\alpha\), \(\beta\)~is \(\in\)-minimal in~\(C\).
...
\begin{rmk}
The next few exercises explore the behavior of the gimel function at singular cardinals.
\end{rmk}

\begin{exer}[5.28]
If \(\kappa\le\gimel(\lambda)\) and \(\cf\kappa\le\cf\lambda\), then \(\gimel(\kappa)\le\gimel(\lambda)\).
\end{exer}
\begin{proof}
\[\gimel(\kappa)=\kappa^{\cf\kappa}\le\gimel(\lambda)^{\cf\kappa}=(\lambda^{\cf\lambda})^{\cf\kappa}=\lambda^{\cf\lambda\mult\cf\kappa}=\lambda^{\cf\lambda}=\gimel(\lambda)\qedhere\]
\end{proof}

\begin{exer}[5.29]
If \(\kappa\)~is singular and \(2^{\cf\kappa}<\kappa\le\lambda^{\cf\kappa}\) for some \(\lambda<\kappa\), then \(\gimel(\kappa)=\gimel(\lambda)\) where \(\lambda\)~is least such that \(\kappa\le\lambda^{\cf\kappa}\).
\end{exer}
\begin{proof}
By Theorem~5.20. \(\gimel(\kappa)=\kappa^{\cf\kappa}=\lambda^{\cf\kappa}\), so it suffices to prove that \(\lambda^{\cf\kappa}=\lambda^{\cf\lambda}=\gimel(\lambda)\). If \(\lambda\le\cf\kappa\), then \(\lambda^{\cf\kappa}=2^{\cf\kappa}<\kappa\le\lambda^{\cf\kappa}\)---a contradiction. If there is \(\mu<\lambda\le\mu^{\cf\kappa}\), then \(\kappa\le\lambda^{\cf\kappa}=\mu^{\cf\kappa}\)---a contradiction since \(\lambda\)~is least with this property. If \(\cf\kappa<\cf\lambda\), then \(\lambda^{\cf\kappa}=\lambda<\kappa\)---a contradiction. Thus \(\lambda^{\cf\kappa}=\lambda^{\cf\lambda}\).
\end{proof}

\section*{Chapter~6}
\begin{rmk}
In the proof of Theorem~6.5, to see that \(F\)~is the unique function on~\(T\) satisfying~(6.6), first observe that if \(F(x)=y_1\) and \(F(x)=y_2\) are witnessed by \(f_1\) and~\(f_2\), respectively, then \(D=\dom f_1\sect\dom f_2\)~is transitive and \(f_1(z)=f_2(z)\) for all \(z\in D\) by \(\in\)-induction on~\(D\), so in particular \(y_1=f_1(x)=f_2(x)=y_2\). It follows that \(F\)~is a function, and if \(x\in\dom F\) then there is a unique witnessing function~\(f_x\) with smallest domain.

If \(x\subset\dom F\), let \(f=\bigunion_{w\in x}f_w\).\footnote{The set \(\{\,f_w\mid w\in x\,\}\) exists by replacement.} Then \(f\)~is a function by \(\in\)-induction as above, \(\dom f=\bigunion_{w\in x}\dom f_w\) is transitive with \(x\subset\dom f\), and \(f(z)=G(f\restriction z)\) for all \(z\in\dom f\). Thus \(f\union\{(x,G(f\restriction x))\}\) witnesses \(F(x)=G(f\restriction x)\), and \(x\in\dom F\). By \(\in\)-induction, it follows that \(\dom F=T\).

Now \(F(x)=f_x(x)=G(f_x\restriction x)=G(F\restriction x)\), so \(F\)~satisfies~(6.6). Finally, if \(F'\)~also satisfies~(6.6), then \(F=F'\) by \(\in\)-induction, so \(F\)~is unique.
\end{rmk}

\begin{exer}[6.1]
\(\rank(x)=\sup\{\,\rank(z)+1\mid z\in x\,\}\).
\end{exer}
\begin{proof}
Let \(\sigma=\sup\{\,\rank(z)+1\mid z\in x\,\}\). If \(z\in x\), then \(\rank(z)<\rank(x)\), so that \(\rank(z)+1\le\rank(x)\). It follows that \(\sigma\le\rank(x)\). Conversely if \(\alpha<\rank(x)\), then by definition \(x\not\in V_{\alpha+1}=\P(V_{\alpha})\), so \(x\not\subset V_{\alpha}\), so there is \(z\in x\) with \(z\not\in V_{\alpha}\) and hence \(\alpha\le\rank(z)\) and \(\alpha<\rank(z)+1\le\sigma\). Since \(\rank(x)\)~is least greater than all \(\alpha<\rank(x)\), it follows that \(\rank(x)\le\sigma\).
\end{proof}

\begin{exer}[6.2]
\(\card{V_{\omega+\alpha}}=\beth_{\alpha}\) for all~\(\alpha\).
\end{exer}
\begin{proof}
By induction on~\(\alpha\). For \(\alpha=0\), we have \(\omega\subset V_{\omega}\) and \(V_n\)~is finite for all \(n<\omega\) by induction on~\(n\), so
\[\aleph_0\le\card{V_{\omega}}=\bigcard{\bigunion_{n<\omega}V_n}\le\al_0=\beth_0\]
If \(\card{V_{\omega+\alpha}}=\beth_{\alpha}\), then
\[\card{V_{\omega+\alpha+1}}=\card{\P(V_{\omega+\alpha})}=2^{\card{V_{\omega+\alpha}}}=2^{\beth_{\alpha}}=\beth_{\alpha+1}\]
If \(\alpha>0\) is limit and \(\card{V_{\omega+\beta}}=\beth_{\beta}\) for all \(\beta<\alpha\), then since \(V_{\omega+\beta}\subset V_{\omega+\alpha}\) for all \(\beta<\alpha\), we have \(\beth_{\beta}\le\card{V_{\omega+\alpha}}\) for all \(\beta<\alpha\) and \(\beth_{\alpha}=\sup\{\,\beth_{\beta}\mid\beta<\alpha\,\}\le\card{V_{\omega+\alpha}}\). Conversely,
\[\card{V_{\omega+\alpha}}=\bigcard{\bigunion_{\beta<\alpha}V_{\omega+\beta}}\le\card{\alpha}\mult\sup\{\,\card{V_{\omega+\beta}}\mid\beta<\alpha\,\}=\card{\alpha}\mult\beth_{\alpha}=\beth_{\alpha}\qedhere\]
\end{proof}

\begin{exer}[6.3]
If \(\kappa\)~is inaccessible, then \(\card{V_{\kappa}}=\kappa\).
\end{exer}
\begin{proof}
\(\kappa\subset V_{\kappa}\), so \(\kappa\le\card{V_{\kappa}}\). Conversely, we claim \(\card{V_{\alpha}}<\kappa\) for \(\alpha<\kappa\). For \(\alpha<\omega\) this is trivial, and for \(\alpha=\omega+\beta\) with \(\beta<\kappa\) we have \(\card{V_{\alpha}}=\beth_{\beta}\) (Exercise~6.2) and \(\beth_{\beta}<\kappa\) by induction on~\(\beta\). Indeed, for \(\beta=0\) this holds since \(\kappa\)~is uncountable. If it holds for~\(\beta\) then it holds for~\(\beta+1\) since \(\kappa\)~is strong limit. If \(\beta>0\) is limit and it holds for all \(\gamma<\beta\), then it holds for~\(\beta\) since \(\kappa\)~is regular. Therefore
\[\card{V_{\kappa}}=\bigcard{\bigunion_{\alpha<\kappa}V_{\alpha}}\le\kappa\mult\sup\{\,\card{V_{\alpha}}\mid\alpha<\kappa\,\}\le\kappa\mult\kappa=\kappa\qedhere\]
\end{proof}

\begin{exer}[6.4]
If \(x\) and~\(y\) have rank~\(\le\alpha\), then
\[\{x,y\}\quad (x,y)\quad x\union y\quad \bigunion x\quad \P(x)\quad x\times y\quad y^x\]
each have rank~\(<\alpha+\omega\).
\end{exer}
\begin{proof}
By assumption \(x,y\in V_{\alpha+1}=\P(V_{\alpha})\), so \(x,y\subset V_{\alpha}\).
\begin{itemize}[itemsep=0pt]
\item \(\{x,y\}\subset V_{\alpha+1}\), so \(\{x,y\}\in V_{\alpha+2}\) and \(\rank(\{x,y\})\le\alpha+1\).
\item \((x,y)=\{\{x\},\{x,y\}\}\subset V_{\alpha+2}\), so \((x,y)\in V_{\alpha+3}\) and \(\rank((x,y))\le\alpha+2\).
\item \(x\union y\subset V_{\alpha}\), so \(x\union y\in V_{\alpha+1}\) and \(\rank(x\union y)\le\alpha\).
\item \(\bigunion x\subset V_{\alpha}\) by transitivity of~\(V_{\alpha}\), so \(\bigunion x\in V_{\alpha+1}\) and \(\rank(\bigunion x)\le\alpha\).
\item \(\P(x)\subset V_{\alpha+1}\), so \(\P(x)\in V_{\alpha+2}\) and \(\rank(P(x))\le\alpha+1\).
\item \(x\times y\subset V_{\alpha}\times V_{\alpha}\subset V_{\alpha+2}\), so \(x\times y\in V_{\alpha+3}\) and \(\rank(x\times y)\le\alpha+2\).
\item \(y^x=\{\,f:x\to y\,\}\subset\P(x\times y)\subset V_{\alpha+3}\), so \(y^x\in V_{\alpha+4}\) and \(\rank(y^x)\le\alpha+3\).\qedhere
\end{itemize}
\end{proof}

\begin{exer}[6.5]
\(\Z,\Q,\R\in V_{\omega+\omega}\).
\end{exer}
\begin{proof}
\(\N=\omega\subset V_{\omega}\). Under standard constructions:
\begin{itemize}[itemsep=0pt]
\item \(\Z\)~is a set of equivalence classes of pairs of elements of~\(\N\)
\item \(\Q\)~is a set of equivalence classes of pairs of elements of~\(\Z\)
\item \(\R\)~is a set of subsets (cuts) of~\(\Q\)
\end{itemize}
To reach each of these sets we must climb up only finitely many steps from~\(V_{\omega}\) in the cumulative hierarchy (see Exercise~6.4).
\end{proof}

\begin{exer}[6.6]
If \(B\)~is the class of sets hereditarily in the class~\(A\), then
\begin{enumerate}[itemsep=0pt]
\item[(i)] \(x\in B\) if and only if \(x\in A\) and \(\tc(x)\subset A\).\footnote{The requirement that \(x\in A\) is essential. For example, if \(A\)~is the class of finite sets, so \(B\)~is the class of hereditarily finite sets, then \(\tc(\omega)=\omega\subset A\) but \(\omega\not\in A\) so \(\omega\not\in B\).}
\item[(ii)] \(B\)~is the largest transitive subclass of~\(A\).
\end{enumerate}
\end{exer}
\begin{proof}
Recall \(B=\{\,x\in A\mid x\subset B\,\}\).
\begin{enumerate}[itemsep=0pt]
\item[(i)] If \(x\in B\), then \(x\in A\). Let \(x_0=x\) and \(x_{n+1}=\bigunion x_n\) for all \(n<\omega\). Then \(x_n\subset B\) for all \(n<\omega\) by induction on~\(n\), and \(\tc(x)=\bigunion_{n<\omega}x_n\subset B\subclass A\). Conversely, if \(T=\tc(x)\union\{x\}\subset A\), then \(T\subset B\) by \(\in\)-induction on~\(T\), so \(x\in B\).
\item[(ii)] By definition, \(B\subclass A\) is transitive. If \(C\subclass A\) is transitive and \(x\in C\), then \(\tc(x)\union\{x\}\subset C\subclass A\), so \(x\in B\) by~(i) and \(C\subclass B\).\qedhere
\end{enumerate}
\end{proof}
