...
\begin{rmk}
An ordinal is a ``finite'' cardinal if and only if it is a ``finite cardinal''. In fact, if a cardinal is ``finite'', then it is a ``finite ordinal'' by the remark on~(2.13) above, and hence it is a ``finite cardinal''. The converse is just the \emph{pigeonhole principle}, which is proved by induction on~\(\omega\).
\end{rmk}
